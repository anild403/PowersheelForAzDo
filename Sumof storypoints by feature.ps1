...
$filepath="C:\Projects\My Project\R4 Remaining work\R4 Remaining Work.xlsx"
...
foreach($relation in $response.value.relations){
        ...
        foreach($userStory in $userStories){
            ...
            if($userStory.fields.'System.State'  -eq "Closed")
            {
                $closedPoints += [int]$userStory.fields.'Microsoft.VSTS.Scheduling.StoryPoints'
                $closedstorycount += 1;
            }
        }
        ...
        $obj = New-Object PSObject -Property @{
                        ...
                        UserStoryCount = $userStories.Count
